...
A self-contained, dependency-free force-directed viewer for the agentic-vl static
...
dotnet tools/vl-map/bin/Release/net10.0/vl-map.dll --project . --out vl-map.json

# 2a. open index.html and drop vl-map.json onto the canvas, or
# 2b. serve the folder so it auto-loads sample-vl-map.json:
python3 -m http.server --directory tools/vl-view 8731   # http://localhost:8731/
```
