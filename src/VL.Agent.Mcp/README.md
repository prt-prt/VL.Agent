...
| URI | What it returns |
|---|---|
| `vl-agent://schema/graph-transaction` | `schemas/graph-transaction.schema.json` |
| `vl-agent://docs/graph-transaction-protocol` | `docs/graph-transaction-protocol.md` |
| `vl-agent://editor/state` | Latest `.agent/editor-state.json`, if present |
...
dotnet build src\VL.Agent.Mcp\VL.Agent.Mcp.csproj -c Release -p:AgenticVlDev=true
```

Point MCP clients at the built exe:

```text
C:\path\to\VL.Agent\src\VL.Agent.Mcp\bin\Release\net10.0\vl-mcp.exe
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File src\VL.Agent.Mcp\dev.ps1
```
...
```text
src\VL.Agent.Mcp\bin\Release\net8.0\vl-mcp.exe
```

Continuous compile checks without starting an MCP transport:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File src\VL.Agent.Mcp\dev.ps1 -Watch
```
...
1. Build/install the package and load `VL.Agent.HDE.vl`.
