...
The prototype paste path is disabled.
...
The MCP server no longer exposes `vvvv_paste`, and `CommandProcessor` rejects stale
`"paste"` requests defensively.

Future node insertion should use a proper editor-command context or another safe
editor-mediated boundary.
