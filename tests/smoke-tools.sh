...
MCP_DLL="src/VL.Agent.Mcp/bin/Release/net10.0/vl-mcp.dll"
...
printf '%s\n' '{"jsonrpc":"2.0","id":3,"method":"resources/list","params":{}}' \
  | "$DOTNET" "$MCP_DLL" \
  | grep -q 'vl-agent://schema/graph-transaction'

echo "== mcp resources/read returns graph transaction schema =="
printf '%s\n' '{"jsonrpc":"2.0","id":4,"method":"resources/read","params":{"uri":"vl-agent://schema/graph-transaction"}}' \
  | "$DOTNET" "$MCP_DLL" \
  | grep -q 'VL.Agent GraphTransaction'
...
printf '%s\n' '{"jsonrpc":"2.0","id":6,"method":"tools/call","params":{"name":"vvvv_context_query","arguments":{"kind":"summary"}}}' \
  | VVVV_AGENT_STATE="$TMP_STATE" "$DOTNET" "$MCP_DLL" \
  | grep -q 'abc:def'
